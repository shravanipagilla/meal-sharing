...
router.get("/:meal_id/reviews", async (request, response) => {
  try {
    // knex syntax for selecting things. Look up the documentation for knex for further info
    const rows = await knex("Review").select("*").where('meal_id',request.params.meal_id);
    if(rows.length !== 0){
    response.send(rows);
  }else {
    response.status(404).json({Message:"No Reviews Related to given mealId"});
  }
  } catch (error) {
    throw error;
  }
...
router.post("/", async (request, response) => {
  try {
    ...
    if(row !== 0){
      ...
      response.status(400).json({Message:"Row not inserted please check your data"});
     }
    }
  } catch (error) {
      throw error;
...
router.put("/:id", async (req, res) => {
 ...
 try {
   // knex syntax for selecting things. Look up the documentation for knex for further info
   const count = await knex('Review').where('id', req.params.id) 
   .update({
    title :putData.title,
...
router.delete("/:id", async (request, response) => {
  try {
    ...
    response.status(400).json({Message:"No  review found with given id to delete"})
  }
